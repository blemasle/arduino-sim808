...
AT_COMMAND(SET_BEARER_SETTING, "AT+SAPBR=3,1,\"%S\",\"%s\"");
AT_COMMAND(GPRS_START_TASK, "AT+CSTT=\"%s\",\"%s\",\"%s\"");
...
{
	sendFormatAT(SFP(AT_COMMAND_SET_BEARER_SETTING), parameter, value);
...
bool SIM808::enableGprs(const char *apn, const char* user, const char *password)
{
	return (sendAT(SFP(TOKEN_CIPSHUT)), waitResponse(65000L, SFP(TOKEN_SHUT_OK)) == 0) &&					//AT+CIPSHUT
		(sendAT(SFP(TOKEN_CGATT), SFP(TOKEN_WRITE), 1), waitResponse(10000L) == 0) &&						//AT+CGATT=1
		...
		(sendAT(SFP(TOKEN_SAPBR), SFP(TOKEN_WRITE), 1, SFP(TOKEN_COMMA), 1), waitResponse(65000L) == 0) &&	//AT+SAPBR=1,1
...
{
	return (sendAT(SFP(TOKEN_CIPSHUT)), waitResponse(65000L, SFP(TOKEN_SHUT_OK)) == 0) &&					//AT+CIPSHUT
		(sendAT(SFP(TOKEN_SAPBR), SFP(TOKEN_WRITE), 0, SFP(TOKEN_COMMA), 1), waitResponse(65000L) == 0) &&	//AT+SAPBR=0,1
		(sendAT(SFP(TOKEN_CGATT), SFP(TOKEN_WRITE), 0), waitResponse(10000L) == 0);							//AT+CGATT=0
}
